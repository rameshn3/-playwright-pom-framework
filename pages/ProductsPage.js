...
class ProductsPage{
    ...
    async clickOnViewProductBtn(productName){
        ...
        for(let i =0 ; i<productCount;i++){
           if(await await this.productName.nth(i).textContent()=== productName){
...
async verifyAddToCartModalContent(){
    expect(await this.addedHeader).toBeVisible();
    expect(await this.addedToCartMsg).toBeVisible();
    expect(await this.viewCartLink).toBeVisible();
    expect(await this.continueShoppingBtn).toBeVisible();
}
